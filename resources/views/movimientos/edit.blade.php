    ...
    <div class="bg-white shadow rounded-lg p-6">
        ...
        <form action="{{ route('movimientos.update', $movimiento->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="bien_id" class="mt-1 block w-full rounded-md border px-3 py-2">
                    @foreach(\App\Models\Bien::orderBy('descripcion')->get() as $bien)
                        <option value="{{ $bien->id }}" {{ $bien->id == $movimiento->bien_id ? 'selected' : '' }}>{{ $bien->codigo }} - {{ $bien->descripcion }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" name="subject_id" class="mt-1 block w-full rounded-md border px-3 py-2" value="{{ $movimiento->subject_id }}" placeholder="ID del sujeto" />
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Tipo</label>
                <input type="text" name="tipo" class="mt-1 block w-full rounded-md border px-3 py-2" value="{{ $movimiento->tipo }}" required />
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Fecha</label>
                <input type="date" name="fecha" class="mt-1 block w-full rounded-md border px-3 py-2" value="{{ optional($movimiento->fecha)->format('Y-m-d') ?? '' }}" required />
            </div>
            ...
            <div>
                ...
                <select name="usuario_id" class="mt-1 block w-full rounded-md border px-3 py-2">
                    @foreach(\App\Models\Usuario::orderBy('nombre')->get() as $u)
                        <option value="{{ $u->id }}" {{ $u->id == $movimiento->usuario_id ? 'selected' : '' }}>{{ $u->nombre_completo }}</option>
                    @endforeach
                </select>
            </div>
